...
function Message({ role, content }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-full bg-pc-premium flex items-center justify-center flex-shrink-0 mr-2 mt-[2px]">
  ...
  if (type === 'create_idea') {
    return (
      <div className="flex justify-start mb-3">
        <div className="w-7 h-7 rounded-full bg-pc-premium flex items-center justify-center flex-shrink-0 mr-2 mt-[2px] opacity-0" />
        <div className="max-w-[78%] bg-[#F5F3FF] border border-[#C4B5FD] rounded-[16px] rounded-bl-[4px] px-4 py-3">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-4 h-4 rounded-full bg-pc-premium flex items-center justify-center flex-shrink-0">
              <svg width="8" height="8" viewBox="0 0 10 10" fill="none">
                <path d="M2 5l2.5 2.5L8 2" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"/>
              </svg>
            </div>
            <span className="text-[11px] font-bold text-pc-premium uppercase tracking-wide">Idée créée</span>
          </div>
          <p className="text-[13px] font-semibold text-pc-ink leading-snug mb-1">"{data.hook}"</p>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] font-medium bg-[#EDE9FE] text-pc-premium px-2 py-[2px] rounded-full">{data.format}</span>
          ...
          <button
            ...
            className="mt-3 text-[11px] font-bold text-pc-premium hover:underline"
          >
            Voir dans Idées →
          </button>
        </div>
    ...
    return (
      <div className="flex justify-start mb-3">
        ...
        <div className="max-w-[78%] bg-[#F0FDF4] border border-[#86EFAC] rounded-[16px] rounded-bl-[4px] px-4 py-3">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-4 h-4 rounded-full bg-pc-success flex items-center justify-center flex-shrink-0">
              <svg width="8" height="8" viewBox="0 0 10 10" fill="none">
                <path d="M2 5l2.5 2.5L8 2" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"/>
              </svg>
            </div>
            <span className="text-[11px] font-bold text-pc-success uppercase tracking-wide">Planifié</span>
          </div>
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[12px] text-pc-success font-medium">{dateLabel}</span>
            <span className="text-[10px] font-medium bg-[#DCFCE7] text-pc-success px-2 py-[2px] rounded-full">{data.format}</span>
          </div>
          {data.brief && <p className="text-[12px] text-pc-ink-3 leading-[1.5]">{data.brief}</p>}
          <button
            onClick={() => onNavigate('/app/calendar')}
            className="mt-3 text-[11px] font-bold text-pc-success hover:underline"
          >
    ...
    <div className="min-h-screen bg-pc-bg flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="max-w-sm w-full text-center">
          <div className="w-16 h-16 rounded-full bg-pc-premium/10 flex items-center justify-center mx-auto mb-5">
            <svg width="28" height="28" viewBox="0 0 20 20" fill="none" stroke={PC_PREMIUM} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <path d="M10 2C6.13 2 3 5.13 3 9c0 2.39 1.19 4.5 3 5.74V17a1 1 0 001 1h6a1 1 0 001-1v-2.26C15.81 13.5 17 11.39 17 9c0-3.87-3.13-7-7-7z"/>
              <path d="M7.5 17.5h5"/>
            </svg>
          </div>
          <span className="inline-block text-[10px] font-bold text-pc-premium border border-pc-premium/30 bg-pc-premium/8 px-3 py-[4px] rounded-full mb-3">
            Exclusif Premium
          </span>
          ...
          <div className="space-y-2 mb-6 text-left">
            {SUGGESTIONS.map((s) => (
              <div key={s} className="flex items-center gap-3 bg-pc-surface border border-pc-border rounded-elem px-4 py-3">
                <div className="w-[6px] h-[6px] rounded-full bg-pc-premium flex-shrink-0" />
          ...
          <button
            onClick={onUpgrade}
            className="w-full py-[14px] rounded-btn bg-pc-premium text-white text-[14px] font-black hover:bg-pc-premium-dark transition-colors"
    ...
    <div className="min-h-screen bg-pc-bg flex flex-col">
      ...
      <div className="bg-pc-surface border-b border-pc-border px-6 pt-7 pb-4 sticky top-0 z-30">
        <div className="max-w-2xl mx-auto lg:max-w-3xl flex items-center gap-3">
          ...
          <div className="w-8 h-8 rounded-full bg-pc-premium flex items-center justify-center flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-[10px] text-pc-premium font-semibold mt-[1px]">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 pb-32 max-w-2xl mx-auto w-full lg:max-w-3xl">
        ...
        {error && (
          <div className="flex justify-start mb-3">
            <div className="max-w-[78%] px-4 py-[10px] rounded-[16px] rounded-bl-[4px] bg-pc-danger-bg border border-pc-danger-light text-[12px] text-pc-danger">
              {error}
            </div>
          </div>
        )}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {SUGGESTIONS.map((s) => (
              <button
                ...
                className="flex-shrink-0 bg-pc-surface border border-pc-border rounded-full px-3 py-[7px] text-[11px] font-semibold text-pc-ink-2 hover:border-pc-premium hover:text-pc-premium transition-colors whitespace-nowrap"
              >
                {s}
              </button>
            ))}
      ...
      >
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 bg-pc-bg border border-pc-border rounded-[20px] px-4 py-[10px] text-[13px] text-pc-ink placeholder:text-pc-ink-4 focus:outline-none focus:border-pc-premium transition-colors resize-none overflow-hidden leading-[1.5]"
            style={{ maxHeight: '100px' }}
          />
          <button
            onClick={() => sendMessage()}
            disabled={!input.trim() || isTyping}
            className="w-9 h-9 rounded-full bg-pc-premium flex items-center justify-center flex-shrink-0 disabled:opacity-40 active:scale-95 transition-all"
          >
